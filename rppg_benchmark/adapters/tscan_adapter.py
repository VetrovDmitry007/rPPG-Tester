"""
Адаптер для модели TS-CAN
https://github.com/ubicomplab/rPPG-Toolbox/blob/main/neural_methods/model/TS_CAN.py
"""
import cv2
import torch
import numpy as np

from rppg_benchmark.models.TSCAN.TS_CAN import TSCAN
from rppg_benchmark.interfaces import IRPPGModel
from rppg_benchmark.rppg_analyzer import RPPGSignalAnalyzer


class TSCANAdapter(IRPPGModel):
    def __init__(self):
        self.model = TSCAN(img_size=72)
        ckpt_path = "rppg_benchmark/models/TSCAN/UBFC-rPPG_TSCAN.pth"
        # ckpt_path = "rppg_benchmark/models/TSCAN/SCAMPS_TSCAN.pth"
        # ckpt_path = "rppg_benchmark/models/TSCAN/PURE_TSCAN.pth"
    ...
    def _preprocess(self, frames):
        ...
        # Обрезаем лишние кадры чтоб общее количество кадров было чётно 20
        print(f'{len(frames)=}')
        # frames =
        preprocessed = []
        ...
        # обрезаем кадры до длины кратной n_segment=20
        n_segment = 20
        n_batch = len(arr) // n_segment
        arr = arr[:n_batch * n_segment]
